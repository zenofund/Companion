...
**Real-time Communication:** Server-Sent Events (SSE) for reliable real-time chat messaging. Uses browser-native EventSource API with automatic reconnection. Backend features MessageBroadcaster class managing SSE connections per booking with heartbeat keep-alives (30s intervals), proper cleanup on disconnect (handles 'close', 'finish', and 'error' events), and session-based authentication. Messages are sent via HTTP POST endpoint and broadcast to connected clients via SSE streams. SSE approach eliminates WebSocket connection multiplication issues in React StrictMode and provides stable, one-way server-to-client streaming.
